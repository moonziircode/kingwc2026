...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const simulateFullTournament = () => {
        // 1. Group Stage (m1 - m72)
        for (let i = 1; i <= 72; i++) {
            leagueData.actualScores[`m${i}`] = {
        ...
        for (let i = 73; i <= 88; i++) {
            ...
            leagueData.actualScores[mId] = { A: scoreA, B: scoreB };
            if (scoreA === scoreB) {
                leagueData.actualScores[`${mId}_penaltyWinner`] = Math.random() < 0.5 ? 'A' : 'B';
            }
        }

        resetAndRecalculateKnockout();

        // 3. Round of 16 (m89 - m96)
        for (let i = 89; i <= 96; i++) {
            const mId = `m${i}`;
            const scoreA = Math.floor(Math.random() * 4);
            const scoreB = Math.floor(Math.random() * 4);
        ...
        alert("Simulasi Berhasil! Seluruh pertandingan Piala Dunia 2026 (fase grup hingga final) telah disimulasikan secara acak realistis. Silakan cek papan klasemen dan bagan fase gugur!");
    };

    // Attach Simulator Listener (Host only)
    const btnSimulate = document.getElementById('btn-simulate-tournament');
    if (btnSimulate) {
        btnSimulate.addEventListener('click', () => {
            if (confirm("Apakah Anda yakin ingin mensimulasikan hasil pertandingan untuk seluruh turnamen secara acak? Ini akan menimpa seluruh skor aktual saat ini.")) {
